...
**simple 页（单题模式 / 顺序刷题·单题模式）**

+ **顺序模式**：从 entry「单题模式」进入传 `mode=order`，不再随机打乱、刷全部题（首页「随机刷题」入口仍随机 + 最多 50 题）
+ **红叉 bug 修复**：原 `answerState.selected.indexOf()` 内联表达式实测不渲染，改为预计算 `item.mark`（ok/err/miss）驱动选项类名与符号，确保红叉✗正常显示
+ **多选漏选蓝勾**：漏选的正确答案显示蓝色✓（`mark-miss`），区别于已选正确的绿色✓
+ **正确答案文字**：单选/判断/多选作答后，选项下方显示「正确答案：XX」
+ **回顾回显**：点题号网格或上下题翻页跳到已答题，重建当时所选 + 正确答案反馈（绿✓/红✗/蓝✓ + 文字）；未答题可当场补答
+ **上一题按钮**：底部按钮区改为「上一题 | 下一题」并排，首题置灰；回顾态可翻页浏览
+ **统计常驻**：底部统计栏去掉 finished 条件，进入即显示「已答 X/总 ✔ Y ❌ Z」，每次作答实时更新（`_updateStats`）；题号网格三态（对绿/错红/未答灰）

**entry 页**

+ 「单题模式」跳转 url 加 `&mode=order`

---

### 20260709 · v0.4.8-practice-ux（刷题体验全面优化）

> 顺序练习/随机刷题的交互体验大幅提升：进度记录、自动完成、自动跳转、滑动切题、视觉反馈。

**question 页（列表模式 / 顺序练习）**

+ 移除「确认提交」按钮，全部答完自动弹出回顾栏（刷题不是考试，不需要交卷）
+ **进度记录**：按题库 ID 存储最后答到的题号，下次进入自动滚动到该位置继续
+ 刷完自动清进度（下次从头开始）
+ 回顾栏：✔ X / ❌ Y / 共 Z + 题号网格跳转

**simple 页（单题模式 / 随机刷题）**

+ **自动跳转**：单选/判断答对 → 绿底 ✓ → 800ms 自动下一题；答错 → 绿底 ✓ 正确项 + 红底 ✗ 错误项 → 手动下一题
+ **视觉反馈**：`answerState` 跟踪答题状态，选项绿底/红底 + ✓/✗ 标记，反馈期间选项 disabled 防误触
+ **滑动切题**：左滑 → 下一题，右滑 → 上一题（阈值 50px），回顾模式也可用
+ 多选题反馈：勾选完毕点「下一题」时显示，延迟 600ms 后跳转

**文档更新**

+ `docs/员工端使用说明.md`：6.1~6.3 重写（列表/单题模式详解 + 反馈 + 滑动 + 回顾 + 错题记录）

---

### 20260709 · v0.4.7（考试删除 + 排序修复 + 文档更新）

> 考试管理加删除功能、考试安排降序排列、PDF 水印字号调整、文档同步更新。

**新增 · `hrDeleteAssessment` 云函数**

+ 级联删除：examEnrollments → historys → assessments
+ 仅允许删除已结束的考试（进行中/候考中拒绝）
+ 二次确认弹窗显示关联数据量（N 人答卷 + M 条成绩）
+ 三按钮并列：[成绩] [结束] [删除]

**修复 · 考试安排排序**

+ `listMyAssessments` 云函数 `orderBy startTime asc` → `desc`，最新考试排最上

**文档更新**

+ `docs/HR-SOP操作手册.md`：更新注册流程（pending + HR 新建）、员工管理（编辑/删除/导入）、新增考试删除和 7.3 节、多选题判分规则
+ `docs/员工端使用说明.md`：首次激活→首次注册（pending 页 + 复制用户码）、多选题判分规则
+ `CHANGELOG.md`：补 v0.4.5 ~ v0.4.7 条目

---

### 20260709 · v0.4.6-registration（员工注册流程改造）

> 用「未注册提示页」替换旧的「激活页」。新用户首次打开小程序显示用户码（openid），复制发给 HR，HR 在后台创建员工并绑定 openid，员工再次打开即可自动登录。

**新建 · `pages/pending` 未注册提示页**

+ 显示 openid + 复制按钮 + 操作说明 + 刷新状态按钮
+ 全局锁定：任何非 active 状态自动 reLaunch 到此页

**改造 · `cloudfunctions/whoAmI`**

+ `unactivated` → `not_registered`

**改造 · `cloudfunctions/hrSetEmployee`**

+ 新建模式：`_id` 为空时创建新员工（姓名/部门/角色/openid）
+ openid 去重校验（编辑和新建均检查）
+ 新建时 openid 可选，留空后续通过编辑绑定

**员工管理**

+ 列表 header 加「+ 新建」按钮
+ `employeeEdit` 支持新建模式 + openid 输入框
+ 编辑模式用户码可编辑（绑定 openid）

---

### 20260709 · v0.4.5-multi-score（多选题分级判分）

> 多选题从严格判分（完全匹配才得分）改为分级判分：选 1 个=0 分、少选但全对=半数分、含错=0 分、完全匹配=满分。
...
+ `submitExam` 云函数：`Math.floor(full/2)` 半分数
+ `simple` 刷题：0.5 分半对
+ `question` 刷题：同上
+ `rightNum`（答对题数）不受影响，仅完全匹配算答对

---

### 20260709 · v0.4.5-dept-picker（目标部门改为勾选列表）

> 新建/编辑考试的目标部门，从原来的手填文本框（输入部门名+逗号分隔）改为点击展开的可勾选列表，消除手写部门名打错字/漏逗号的风险。

**改造 · `pages/hr/assessmentEdit`**

+ 目标部门行从 `<input>` 改为可展开的 checkbox 列表（6 个部门 + 全选/取消全选）
+ 默认全部勾选（`targetDepts = []`，语义 = 全员）
+ 点击行展开/收起列表，箭头旋转动画
+ 单独勾选/取消，全选归一化为空数组
+ 最少保留 1 个部门（全部取消时 toast 阻止）
+ 编辑回填自动恢复勾选状态
+ 存储格式不变（`targetDepts: string[]`），云函数零改动

**文档更新**

+ `docs/HR-SOP操作手册.md`：更新 6.2 新建考试配置表 + 3.2~3.3 员工管理操作说明（编辑/删除按钮）
